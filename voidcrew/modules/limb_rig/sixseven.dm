...
/// A big number floating over one hand.
/obj/effect/abstract/sixseven_number
	mouse_opacity = MOUSE_OPACITY_TRANSPARENT
	vis_flags = VIS_INHERIT_PLANE
	appearance_flags = RESET_TRANSFORM|RESET_COLOR|RESET_ALPHA|PIXEL_SCALE|TILE_BOUND
	layer = FLOAT_LAYER
	maptext_width = 16
	maptext_height = 16
...
/mob/living/carbon/proc/show_sixseven()
	...
	for(var/digit in list("6", "7"))
		var/obj/effect/abstract/sixseven_number/number = new
		number.maptext = MAPTEXT("<span style='font-size: 12px; color: [digit == "6" ? "#ffd84a" : "#7ae0ff"]'>[digit]</span>")
	...
	addtimer(CALLBACK(src, PROC_REF(clear_sixseven), numbers), SIXSEVEN_DURATION + 0.3 SECONDS, TIMER_DELETE_ME)

/mob/living/carbon/proc/clear_sixseven(list/numbers)
	for(var/obj/effect/abstract/sixseven_number/number as anything in numbers)
		vis_contents -= number
		qdel(number)
